...
function dbSaveInvoice(invoice) {
  ...
  if (!_isValidInvoice(invoice))
    throw new Error('invoice is invalid cannot save in DB.');
...
async function dbGetInvoices() {
  ...
  const querySnapshot = await db
    .collection(invoicesDbCollectionName)
...
function dbDeleteInvoice(invoiceId) {
  if (!invoiceId)
    throw new Error('invoiceId is undefined cannot delete in DB.');
